...
resource "aws_instance" "FRONTEND_C" {
    ...
    instance_type = "t2.medium"
    tags = {
        Name = "${var.environment}-FRONTEND003"
        Environment = "${var.environment}"
        sshUser = "ubuntu"
    }
}

resource "aws_instance" "SIDEKIQ_A" {
    ami = "${lookup(var.aws_ubuntu_amis,var.region)}"
    instance_type = "t2.medium"
    tags = {
        Name = "${var.environment}-SIDEKIQ001"
        Environment = "${var.environment}"
        sshUser = "ubuntu"
    }
    subnet_id = "${aws_subnet.public-frontend_az-a.id}"
...
resource "aws_instance" "SIDEKIQ_B" {
    ami = "${lookup(var.aws_ubuntu_amis,var.region)}"
    instance_type = "t2.medium"
    tags = {
        Name = "${var.environment}-SIDEKIQ002"
        Environment = "${var.environment}"
        sshUser = "ubuntu"
    }
    subnet_id = "${aws_subnet.public-frontend_az-b.id}"
...
resource "aws_instance" "SIDEKIQ_C" {
    ami = "${lookup(var.aws_ubuntu_amis,var.region)}"
    instance_type = "t2.medium"
    tags = {
        Name = "${var.environment}-SIDEKIQ003"
        Environment = "${var.environment}"
        sshUser = "ubuntu"
    }
    subnet_id = "${aws_subnet.public-frontend_az-b.id}"
...
resource "aws_instance" "SIDEKIQ_D" {
    ami = "${lookup(var.aws_ubuntu_amis,var.region)}"
    instance_type = "t2.medium"
    tags = {
        Name = "${var.environment}-SIDEKIQ004"
        Environment = "${var.environment}"
        sshUser = "ubuntu"
    }
    subnet_id = "${aws_subnet.public-frontend_az-b.id}"
...
resource "aws_instance" "GITALY_A" {
    ami = "${lookup(var.aws_ubuntu_amis,var.region)}"
    instance_type = "t2.medium"
    subnet_id = "${aws_subnet.public-frontend_az-a.id}"
    key_name = "${aws_key_pair.keypair.key_name}"
